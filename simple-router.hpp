...
public:
  ...
  void
  process_arp_reply(arp_hdr* arp_packet_hdr, const Interface* iface);

  void
  process_arp_request(arp_hdr* arp_packet_hdr, const Interface* iface);

  void
  process_arp_packet(std::vector<unsigned char>& packet, const Interface* iface);

  void
  send_arp_request(uint32_t ip, const Interface* iface);

  void
  forward_packet(std::vector<unsigned char>& packet, uint32_t target_ip, const Interface* iface);

  void
  process_ip_packet(std::vector<unsigned char>& packet, const Interface* iface);
